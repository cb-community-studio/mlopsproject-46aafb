...
import modelsFakerFactory from "./modelsFakerFactory";
...
const ModelsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = modelsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("models").create(elem));
...
export default connect(null, mapDispatch)(ModelsFakerDialogComponent);
