...
import modelslistFakerFactory from "./modelslistFakerFactory";
...
const ModelslistFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = modelslistFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("modelslist").create(elem));
...
export default connect(null, mapDispatch)(ModelslistFakerDialogComponent);
